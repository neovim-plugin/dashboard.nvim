...
function M.run_command(cmd, el, state, line)
  ...
  vim.uv
    .new_async(vim.schedule_wrap(function()
      ...
      vim.api.nvim_set_option_value("buflisted", false, { buf = wininfo[1] })
      vim.api.nvim_command('let b:term_title ="dashboard_terminal" ')
    end))
    :send()
end
...
function dashboard.layout_element.terminal(el, conf, state)
  ...
  return dashboard.layout_element.padding({ type = "padding", val = el.height }, conf, state)
end

function dashboard.keymaps_element.terminal(_, _, _) end
