...
local function draw_press(row, col, state)
  ...
  -- todo: represent this in the dashboard layout, somehow
...
function dashboard.queue_press(state)
...
function dashboard.align_center(tbl, state)
...
function dashboard.pad_margin(tbl, state, margin, shrink)
  local longest = longest_line(tbl)
  local left
  if shrink then
...
function dashboard.highlight(state, end_ln, hl, left, el)
...
function dashboard.resolve(to, el, opts, state)
...
function layout_element.text(el, conf, state)
  if type(el.val) == "function" then
    return dashboard.resolve(layout_element.text, el, conf, state)
  end
  local val
  if type(el.val) == "string" then
    val = {}
  ...
  if margin and (position ~= "center") then
    local left
    val, left = dashboard.pad_margin(
      val,
      state,
      margin,
      if_nil(vim.tbl_get(el, "opts", "shrink_margin"), true)
    )
    padding.left = padding.left + left
  end
  if position == "center" then
    local left
    val, left = dashboard.align_center(val, state)
    padding.left = padding.left + left
  end
  local el_hl = vim.tbl_get(el, "opts", "hl")
  if type(el.val) == "string" then
    if el_hl then
      hl = dashboard.highlight(state, state.line, el_hl, padding.left, el)
    end
    state.line = state.line + 1
  else
    local end_ln = state.line + #el.val
    if el_hl then
      hl = dashboard.highlight(state, end_ln, el_hl, padding.left, el)
    end
    state.line = end_ln
  end
  return val, hl
...
function layout_element.button(el, conf, state)
  ...
  if vim.tbl_get(conf, "opts", "margin") and (vim.tbl_get(opts, "position") ~= "center") then
    ...
    val, left = dashboard.pad_margin(
      ...
      state,
      conf.opts.margin,
      if_nil(vim.tbl_get(opts, "shrink_margin"), true)
    )
  ...
  if el.opts and el.opts.hl_shortcut then
    ...
    if el.opts.align_shortcut == "right" then
      hl = dashboard.highlight(state, state.line, hl, #el.val + math.max(0, padding.center), el)
    else
      hl = dashboard.highlight(state, state.line, hl, padding.left, el)
    end
  end
  ...
  if el.opts and el.opts.hl then
    ...
    list_extend(hl, dashboard.highlight(state, state.line, el.opts.hl, left, el))
  end
  state.line = state.line + 1
  return val, hl
end
...
function layout_element.group(el, conf, state)
  if type(el.val) == "function" then
    return dashboard.resolve(layout_element.group, el, conf, state)
  end
...
function keymaps_element.group(el, conf, state)
  if type(el.val) == "function" then
    dashboard.resolve(keymaps_element.group, el, conf, state)
  end
...
-- stylua: ignore start
local function enable_dashboard(conf, state)
    ...
    local group_id = vim.api.nvim_create_augroup('dashboard_temp', { clear = true })
    ...
    vim.api.nvim_create_autocmd({'WinClosed'}, {
        group = group_id,
        buffer = state.buffer,
        callback = dashboard.handle_window,
    })
    ...
    vim.api.nvim_create_autocmd('CursorMoved', {
        ...
        buffer = state.buffer,
        callback = function()
            dashboard.move_cursor(active_window(state))
        end,
    })
        ...
        if if_nil(conf.opts.redraw_on_resize, true) then
            if vim.version().api_level >= 11 then
                vim.api.nvim_create_autocmd('WinResized', {
                    group = group_id,
                    callback = function() dashboard.redraw(conf, state) end,
                })
            else
                ...
                vim.api.nvim_create_autocmd('CursorMoved', {
                    ...
                    callback = function()
                        ...
                        then dashboard.redraw(conf, state)
                        end
                    end,
                })
            end
...
local function should_skip_dashboard()
    ...
    -- Do not open dashboard if the current buffer has any lines (something opened explicitly).
...
function dashboard.draw(conf, state)
  ...
  if vim.api.nvim_get_current_win() == active_win then
    if #cursor_jumps ~= 0 then
      ...
      -- like WinEnter will say 'dashboard' is the current open buffer
...
function dashboard.move_cursor(window)
  if #cursor_jumps ~= 0 then
    local cursor = vim.api.nvim_win_get_cursor(window)
    local closest_ix, closest_pt =
      closest_cursor_jump(cursor, cursor_jumps, cursor_jumps[cursor_ix])
...
function dashboard.redraw(conf, state)
  if (conf == nil) and (state == nil) then
    local buffer = vim.api.nvim_get_current_buf()
    local dashboard_prime = vim.tbl_get(dashboard_state, buffer) or head(dashboard_state)
    if dashboard_prime == nil then
      return
    end
    conf = dashboard.default_config
    state = dashboard_prime
  end
  dashboard.draw(conf, state)
end

function dashboard.close(ev)
  dashboard_state[ev.buf] = nil
  cursor_ix = 1
  cursor_jumps = {}
  vim.api.nvim_del_augroup_by_id(ev.group)
  vim.api.nvim_exec_autocmds("User", { pattern = "DashboardClosed" })
end

-- @param on_vimenter: ?bool optional
-- @param fon: ?table optional
function dashboard.start(on_vimenter, conf)
  local window = vim.api.nvim_get_current_win()

  local buffer
  if on_vimenter then
    if should_skip_dashboard() then
      return
    end
  ...
  conf = conf or dashboard.default_config
  ...
  for _, k in ipairs(normalize_keymaps(conf.opts.keymap.queue_press)) do
    vim.keymap.set("n", k, function()
      dashboard.queue_press(state)
    end, { noremap = false, silent = true, buffer = state.buffer })
  end

  enable_dashboard(conf, state)
...
function dashboard.setup(config)
  ...
  dashboard.default_config = config

  vim.api.nvim_create_user_command("Dashboard", function(_)
    dashboard.start(false, config)
  end, {
    bang = true,
    desc = 'require"dashboard".start(false)',
    nargs = 0,
    bar = true,
  })

  vim.api.nvim_create_user_command("DashboardRedraw", function(_)
    dashboard.redraw()
  end, {
    bang = true,
    desc = 'require"dashboard".redraw()',
    nargs = 0,
    bar = true,
  })
  vim.api.nvim_create_user_command("DashboardRemap", function(_)
    local buffer = vim.api.nvim_get_current_buf()
    local dashboard_prime = vim.tbl_get(dashboard_state, buffer) or head(dashboard_state)
    if dashboard_prime == nil then
      return
    end
    local conf = dashboard.default_config
    local state = dashboard_prime
    keymaps(conf, state)
  end, {
    bang = true,
  ...
  local group_id = vim.api.nvim_create_augroup("dashboard_start", { clear = true })
  vim.api.nvim_create_autocmd("VimEnter", {
    ...
    callback = function()
      if config.opts.autostart then
        dashboard.start(true, config)
      end
    end,
  })
end

dashboard.layout_element = layout_element
dashboard.keymaps_element = keymaps_element

function dashboard.handle_window(x)
  local dashboard_instance = dashboard_state[x.buf]
  local current_win = vim.api.nvim_get_current_win()
  if dashboard_instance then
    local wins = vim.tbl_filter(function(win)
      return (vim.api.nvim_win_get_buf(win) == x.buf) and (win ~= current_win)
    end, vim.api.nvim_list_wins())
    dashboard_instance.windows = wins
  end
end
